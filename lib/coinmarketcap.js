...
function cmc(options) {
	...
	self.getTicker = function(options, callback) {
		if (
			!options.pair.includes('BCH_USD') &&
			!options.pair.includes('BTC_USD') &&
			!options.pair.includes('LTC_USD') &&
			!options.pair.includes('XRP_USD') &&
			!options.pair.includes('ETH_USD')
		) {
			const ticker = {
				timestamp: util.timestampNow(),
				error: 'Error 404: {"message":"NotFound"}',
				data: [],
			};
